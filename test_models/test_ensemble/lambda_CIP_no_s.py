...
# ==========< Ensemble CIP  >================================================================================
def get_ensemble_cip(_name, _beta, _lamb, fast=True):
    ensemble = ensembleCIP_classification(name=_name,
                                          ...
                                          beta=_beta, lamb=_lamb, s=None, lr=lr)
...
bb, ll = np.meshgrid(beta, lamb)
parameters = list(zip(bb.flatten(), ll.flatten()))



path_data = 'test_params_cip_no_sigma/'
...
if not os.path.exists(file_scores):
    Logger().reset()
    for b, l in Logger().progressbar(it=parameters, end='Finish'):
        Logger().log_disable()
        model = get_ensemble_cip(_name=name, _beta=b, _lamb=l)
        ...
        scores[(b, l)] = {'best_score': best_score, 'list_score': list_score}
        Logger().log_enable()
...
def get_mean_score(_scores, _s, train=True):
    best_scores = []
    beta = []
    lamb = []
    i = 0 if train else 1
...
X, Y, Z = get_mean_score(scores, False)
