...
% bp_ffnet2 	 - Trains the two layer network with backpropagation.
% ffnet2 	 - Simulates the two layer network.
...
[W1,B1,W2,B2,mse] = bp_ffnet2(W1,B1,W2,B2,inpT,desT,TP);
...
% We can also generate a prediction iteratively. The following loop takes
% the last window of inputs in the time series, and generates a prediction
% of the time series for 1000 steps into the future by using a succession
% of one-step-ahead predictions.

PP = 1000;
inpSeed = inpT(Np-sum(Taps)-1 : Np-1);  % the last window in the data
Nw = length(inpSeed);			% the length of the window
predTMP = [inpSeed zeros(Ni,PP)];	% append some zeros
 for k = 1:PP;
   inpP = predTMP(k-Taps+Nw-1:k+Nw-1);	% use the previous prediction
   p1  = ffnet2(W1,B1,W2,B2,inpP);	% to generate the next one.
   predTMP(:,k+Nw) = p1(:,Taps+1);	% store the prediction.
 end
pred = predTMP(:, Nw+1:Nw+PP);		% look at the predicted series
setfsize(500,500);
plot(pred(2:PP),pred(1:PP-1),'.');	% plot the phase portrait

pause % Strike any key to continue...
clc

% The computations on the previous page can also be completed by using the
% the following .m function:  iterate_ffnet2(W1,B1,W2,B2,inpSeed,PP), where
% inpSeed is the initializing window of the time series data, and PP is the
% number of iterative one-step-ahead predictions to compute.

echo off
disp('End of DEMO1FF2')
